...
#   geom_line(color = "#f3a328", size = 2) +
#   geom_point() +
#   scale_x_continuous(breaks=c(0, 100000, 200000, 300000, 400000, 500000, 600000),
#                      labels=c("0", "10", "20", "30", "40", "50", "60")) +
#   ylab("total production (in M kg)") +
#   xlab("number of honeyproducing colonies per State [in thousand]") +
#   theme_bw()+
#   background_grid(major = "xy", minor="y", size.major = 0.2)
```
...
scatter_ggplot = 
  meanregion_year %>%
  ggplot(aes(x = year, y = yieldpercol_kg, color = price_per_kg, fill= Region)) +
...
base <- total_production %>%
  plot_ly(x = ~price_per_kg, y = ~nAllNeonic_kg/10000, size = ~yieldpercol_kg, 
...
```{r}
# region_yield <- with(total_production, tapply(yieldpercol_kg, INDEX = Region, mean))
# total_production$Region <- factor(
#   total_production$Region, levels = names(sort(region_yield))
# )
# 
# region_yield %>%
#   add_markers(data = total_production, frame = ~Region) %>%
#   hide_legend() %>%
...
empty_bar=4
to_add = data.frame( matrix(NA, empty_bar*nlevels(total_production$group), ncol(total_production)) )
colnames(to_add) = colnames(total_production)
to_add$group=rep(levels(data$group), each=empty_bar)
data=rbind(total_production, to_add)
total_production=total_production %>% arrange(group)
total_production$id=seq(1, nrow(total_production))
 
# Get the name and the y position of each label
label_data=total_production
...
# Make the plot
p = ggplot(total_production, aes(x=as.factor(yieldpercol_kg), y=StateName, fill=Region)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  coord_polar() + 
  geom_text(data=label_data, aes(x=id, y=yieldpercol_kg+10, label=individual, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) 
