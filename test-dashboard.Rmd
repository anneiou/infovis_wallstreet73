...
honeyperstate <- total_production %>%
    group_by(StateName, totalprod_kg) %>%
...
ggplot(data=honeyperstate,
       ...
       coord_flip()+
       labs(
         title="Total honey produced per state from 1991 to 2017", 
         subtitle = "Percentage of the total production (bars) and total amount (x-axis)",
         x="Top 10 United States",
...
scatter_ggplot = 
  total_production %>%
  group_by(Region, year, nAllNeonic_kg) %>%
  mutate(neonicyear = sum(nAllNeonic_kg)) %>%
  ggplot(aes(x = year, y = neonicyear, color = Region, fill= Region)) +
  geom_point() +
  #geom_line() +
  labs(
...
year_col <- total_production %>%
  group_by(year) %>%
  summarise(colony_year=sum(numcol))
...
pesticides_year_total <- merge(year_col, pesticides_year_neonics)

ggplot(data=pesticides_year_total,
                 aes(x = year)
                 ) + 
             geom_line(aes(y = colony_year, colour = "number of colonies")) + 
             geom_line(aes(y = neonics_year*30, colour = "Neonic Pesticides")) + 
             scale_y_continuous(breaks=c(
                              ...
                              #  labels=c( 
                                      #   "0", "500.000", "100.000", "150.000", "200.000", "250.000", "300.000", "350.000"
                                      #   ),
                                sec.axis = sec_axis(~./30,
                                                    name = "Neonics applied in the US [million kg]",
                                                    breaks=c(0, 500000, 1000000, 1500000,
  ...
  geom_point(aes(y = colony_year, colour = "number of colonies")) + 
  geom_point(aes(y = neonics_year*30, colour = "Neonic Pesticides")) + 
  scale_colour_manual(values = c("green", "#e5b740")) + 
  labs(y = "Number of honeyproducing colonies in the US", x = "Year", colour = "Parameter") +
  ggtitle("Year 2000 - What happened to our bees?") +
  theme_minimal()
```
